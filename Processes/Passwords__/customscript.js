...
var ctrlChangedWhenNoMasterPassword = null;

var PasswordGenerator = /** @class */ (function () {
    function PasswordGenerator() {
        this.charsetNumbers = "0123456789";
        this.charsetLowercase = "abcdefghijklmnopqrstuvwxyz";
        this.charsetUppercase = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
        this.charsetSymbols = "!\"#$%&'()*+,-./:;<=>?@[\\]^_`{|}~ ";
        this.initCrypto();
    }
    // Returns a random integer in the range [0, n) using a variety of methods
    PasswordGenerator.prototype.randomInt = function (n) {
        var x = this.randomIntMathRandom(n);
        x = (x + this.randomIntBrowserCrypto(n)) % n;
        return x;
    };
    // Not secure or high quality, but always available
    PasswordGenerator.prototype.randomIntMathRandom = function (n) {
    ...
    PasswordGenerator.prototype.randomIntBrowserCrypto = function (n) {
    ...
    PasswordGenerator.prototype.initCrypto = function () {
        ...
        if (!("getRandomValues" in this.cryptoObject) || (typeof Uint32Array != "function")) {
            this.cryptoObject = null;
        }
    };
    PasswordGenerator.prototype.generatePassword = function (passwordLength, useUppercase, useLowercase, useNumbers, useSymbols) {
        // Gather the character set
        var charsetStr = "";
        if (useUppercase) {
            charsetStr += this.charsetUppercase;
        }
        if (useLowercase) {
            charsetStr += this.charsetLowercase;
        }
        if (useNumbers) {
            charsetStr += this.charsetNumbers;
        }
        if (useSymbols) {
            charsetStr += this.charsetSymbols;
        }
        ...
        if (charset.length == 0) {
            ...
            var length_1 = passwordLength;
            if (length_1 < 0) {
                alert("Negative password length");
            }
            else if (length_1 > 1000) {
                ...
                var entropy = Math.log(charset.length) * length_1 / Math.log(2);
                var entropystr = void 0;
                if (entropy < 70) {
...
function commitMasterPasswordDialog_CB(dialog, tabId, event, control)
{
	...
	{
		var control = ctrlClickedWhenNoMasterPassword;
		ctrlClickedWhenNoMasterPassword = null;
		control.OnClick();
	}
	
	if (masterPassword && ctrlChangedWhenNoMasterPassword)
	{
		var control = ctrlChangedWhenNoMasterPassword;
		ctrlChangedWhenNoMasterPassword = null;
		control.OnChange();
	}
}
...
{
	Popup.Dialog(Language.Translate("_Master password"), null, fillMasterPasswordDialog_CB, commitMasterPasswordDialog_CB);
}
...
var b64DecodeUnicode = function(str) {
    var result = sjcl.codec.base64.toBits(str);
	return result;
};

Controls.Generate_Button__.OnClick = function()
{
	// Generate new password
	var gen = new PasswordGenerator();
	var passwordLength = Controls.Required_Length__.GetValue();
	var useUppercase = Controls.Uppercase_Required__.IsChecked();
	var useLowercase = Controls.Lowercase_Required__.IsChecked();
	var useNumbers = Controls.Numbers_Required__.IsChecked();
	...
	if (encryptPassword(password, Controls.Notes__.GetValue()))
	{
		Controls.ClearPassword__.SetValue(Array(parseInt(passwordLength) + 1).join("*"));
	}
	else
	{
		ctrlClickedWhenNoMasterPassword = this;
	}
};
...
var encryptPassword = function(password, notes)
{
	...
	try
	{
		cipherText = sjcl.mode.gcm.encrypt(aes, passwordBytes, ivBytes, additionalData, tagLength);
		
		encryptedPassword = b64EncodeUnicode(cipherText);
...
Controls.Notes__.OnChange = function()
{
	...
	{
		ctrlChangedWhenNoMasterPassword = control;
	}
};
...
{
	return encryptPassword(password, notes);
};
...
Controls.ClearPassword__.OnChange = function()
{
	...
	if (!onNewManualPassword(control.GetValue(), Controls.Notes__.GetValue()))
	{
		ctrlChangedWhenNoMasterPassword = control;
	}
};

Controls.ForgetMasterPasswordButton__.OnClick = function()
{
	var control = this;
	masterPassword = null;
